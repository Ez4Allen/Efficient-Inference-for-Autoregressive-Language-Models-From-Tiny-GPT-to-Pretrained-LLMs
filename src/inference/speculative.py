...
def _draft_propose(
    ...
    *,
    ...
    caller consumes that last token only after target verification.  This
    avoids an unnecessary draft forward pass when the target rejects earlier in
    the candidate block while still preserving cache correctness.
    """
...
def greedy_speculative_decode(
    ...
    """Greedy speculative decoding with persistent draft and target KV caches.

    The output is token-identical to greedy target-only decoding.  Both models
    prefill the prompt exactly once.  After each verification round, rejected
    cache suffixes are cropped and the target correction or bonus token is fed
    to both models so their caches stay synchronized.
    """
    ...
    draft_outputs = draft_model(input_ids=current_ids, use_cache=True)
    ...
    target_outputs = target_model(input_ids=current_ids, use_cache=True)
